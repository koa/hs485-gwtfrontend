...
public class FloorComposite extends Composite {
  private int compositeWidth  = 500;
  private int compositeHeight = 300;
  ...
  public FloorComposite() {
    ...
    svgImage.setHeight("100%");
    initWidget(svgImage);
    Window.addResizeHandler(new ResizeHandler() {

      @Override
      public void onResize(final ResizeEvent event) {
        compositeWidth = event.getWidth();
        compositeHeight = event.getHeight();
        scaleToFit();
      }
    });
  ...
  public void redrawAllIcons() {
    ...
    for (final InputDevice inputDevice : currentFloor.getInputDevices()) {
      ...
      if (Float.isInfinite(position.getX()))
        position.setX(1f);
      if (Float.isInfinite(position.getY()))
        position.setY(1f);
  ...
  public void repaintAll() {
    if (currentFloor != null && currentFloor.getDrawing() != null && currentFloor.getDrawing()
                                                                                 .getFileDataContent() != null)
  ...
  private void scaleToFit() {
    ...
    final int imageWidth = compositeWidth;
    final int imageHeight = compositeHeight;
    final float drawingWidth = svgDrawingWidth;
    final float drawingHeight = svgDrawingHeight;
    final float yScale = imageHeight / drawingHeight;
    final float xScale = imageWidth / drawingWidth;
    scale = Math.max(Math.min(yScale, xScale), 0.001f);
